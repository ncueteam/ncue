...
async def sendDHTData(uuid, humidity, temperature):
    post_data = {'device_id': uuid,'humidity': humidity, 'temp': temperature}
    response = urequests.post("http://frp.4hotel.tw:25580/api/device/info/updateDeviceDatas", headers = {'content-type': 'application/json'}, data = post_data)
    print(response.text)
